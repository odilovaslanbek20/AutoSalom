...
const Breadcrumb = () => {
	...
	return (
		<div className='flex items-center gap-[9px] text-sm text-gray-600 font-[Noto Sans] my-[10px] max-[500px]:my-[15px]'>
			<div className='group'>
				<Link to='/' className='text-[12px] font-normal font-["Noto Sans"] leading-[24px] uppercase'>
					Bosh sahifa
				</Link>
			...
			{pathnames.map((name, index) => {
				...
				return (
					<span key={index} className='flex items-center gap-2'>
						...
						<div className='group'>
							<Link to={routeTo} className='text-[12px] font-normal font-["Noto Sans"] leading-[24px] uppercase'>{formattedName}</Link>
							<div className='w-0 group-hover:w-full transition-all h-[1px] bg-[#00B894]'></div>
						</div>
					</span>
				)
